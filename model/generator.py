...
class Generator(nn.Module):
    def __init__(self, in_channels, img_cond_channels, feat_channels, feat_cond_channels, noise_channels, dropout):
        super().__init__()
        ...
        self.dec_layers = nn.ModuleList([
            nn.Sequential(
                nn.ConvTranspose3d(out_c * 2, out_c * 2, kernel_size=(3, 3, 1), padding=(1, 1, 0)),     # 時間方向に畳み込みを適用してもいいかも
                nn.ConvTranspose3d(out_c * 2, in_c, kernel_size=(2, 2, 1), stride=(2, 2, 1)),
                nn.BatchNorm3d(in_c),
                nn.ReLU(),
                nn.Dropout(dropout),
        ...
        self.dec_last_layer = nn.Sequential(
            nn.ConvTranspose3d(out_cs[0] * 2, out_cs[0] * 2, kernel_size=(3, 3, 1), padding=(1, 1, 0)),
            nn.ConvTranspose3d(out_cs[0] * 2, in_cs[0], kernel_size=(2, 2, 1), stride=(2, 2, 1)),
