...
from model.discriminator import FrameDiscriminator, SequenceDiscriminator, SyncDiscriminator
...
def make_model(cfg, device):
    gen = Generator(cfg.model.in_channels, cfg.model.img_cond_channels, cfg.model.feat_channels, cfg.model.feat_cond_channels, cfg.model.noise_channels, cfg.train.gen_dropout)
    frame_disc = FrameDiscriminator(int(cfg.model.in_channels * 2), cfg.train.frame_disc_dropout)
    seq_disc = SequenceDiscriminator(cfg.model.in_channels, cfg.model.feat_channels, cfg.train.seq_disc_dropout)
...
def train_one_epoch(gen, frame_disc, seq_disc, sync_disc, train_loader, loss_f, opt_gen, opt_frame_disc, opt_seq_disc, opt_sync_disc, device, cfg):
    ...
    for batch in train_loader:
        ...
        # sequence
        fake_seq = seq_disc(out, feature)
        real_seq = seq_disc(lip, feature)
        seq_loss = loss_f.seq_bce_loss_d(fake_seq, real_seq, data_len, lip.shape[-1])
        ...
        if cfg.train.gen_opt_step:
            ...
            seq_loss = loss_f.seq_bce_loss_g(fake_seq, data_len, lip.shape[-1])
