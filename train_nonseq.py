...
from model.discriminator import FrameDiscriminator, SyncDiscriminator, FrameDiscriminatorUNet
...
def save_checkpoint(
    gen, frame_disc, sync_disc, 
    opt_gen, opt_frame_disc, opt_sync_disc,
    scheduler_gen, scheduler_frame_disc, scheduler_sync_disc,
    epoch, ckpt_path):
    torch.save(
        {
            'gen': gen.state_dict(),
...
def make_model(cfg, device):
    ...
    return gen.to(device), frame_disc.to(device), sync_disc.to(device)
...
def train_one_epoch(gen, frame_disc, sync_disc, train_loader, loss_f, opt_gen, opt_frame_disc, opt_sync_disc, device, cfg):
    epoch_loss_gen = 0
    ...
    for batch in train_loader:
        ...
        # frame disc
        if int(torch.min(data_len)) < lip.shape[-1]:
            frame_idx = torch.randint(0, int(torch.min(data_len)), (1,))    # マスクしたフレームは使わないようにする
        else:
        ...
        if cfg.train.gen_opt_step:
            ...
            with torch.no_grad():
                # frame disc
                if int(torch.min(data_len)) < lip.shape[-1]:
                    frame_idx = torch.randint(0, int(torch.min(data_len)), (1,))
                else:
                    frame_idx = torch.randint(0, lip.shape[-1], (1,))
            ...
            gen_loss = cfg.train.frame_weight * frame_loss \
                 + cfg.train.sync_weight * sync_loss + cfg.train.l1_weight * l1_loss
    ...
    return epoch_loss_gen, epoch_loss_frame_disc, epoch_loss_sync_disc


def calc_val_loss(gen, frame_disc, sync_disc, val_loader, loss_f, device, cfg):
    epoch_loss_gen = 0
    epoch_loss_frame_disc = 0
    epoch_loss_sync_disc = 0
    iter_cnt = 0
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        gen, frame_disc, sync_disc = make_model(cfg, device)
        ...
        for epoch in range(cfg.train.max_epoch - last_epoch):
            ...
            # train
            epoch_loss_gen, epoch_loss_frame_disc, epoch_loss_sync_disc = train_one_epoch(
                gen=gen,
            ...
            # validation
            epoch_loss_gen, epoch_loss_frame_disc, epoch_loss_sync_disc = calc_val_loss(
                gen=gen,
                frame_disc=frame_disc,
                sync_disc=sync_disc,
